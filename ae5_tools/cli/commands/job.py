import click

from ..login import cluster_call, login_options
from ..utils import add_param, ident_filter
from ..format import print_output, format_options
from ...identifier import Identifier


@click.group(short_help='info, list, schedule, run, patch, pause, unpause, delete',
             epilog='Type "ae5 job <command> --help" for help on a specific command.')
@format_options()
@login_options()
def job():
    pass


@job.command(short_help='List available jobs.')
@ident_filter('job')
@format_options()
@login_options()
def list():
    cluster_call('job_list', cli=True)


@job.command(short_help='Get information about a single job.')
@click.argument('job')
@format_options()
@login_options()
def info(job):
    cluster_call('job_info', job, cli=True)


@job.command(short_help='List the runs executed for a single job.')
@click.argument('job')
@format_options()
@login_options()
def runs(job):
    cluster_call('job_runs', job, cli=True)


@job.command(short_help='Pause a job.')
@click.argument('job')
@format_options()
@login_options()
def pause(job):
    cluster_call('job_pause', job, cli=True)


@job.command(short_help='Unpause a job.')
@click.argument('job')
@format_options()
@login_options()
def unpause(job):
    cluster_call('job_unpause', job, cli=True)


def _variables(variable, clear_variables):
    if variable:
        if clear_variables:
            raise click.UsageError('Cannot supply both --variable and --clear-variables')
        invalids = [x for x in variable if '=' not in x]
        if invalids:
            msg = ['One or more of the --variable options is invalid:'] + invalids
            raise click.UsageError(f'\n  - '.join(msg))
        variable = (z.split('=', 1) for z in variable)
        variable = dict((x.rstrip(), y.lstrip()) for x, y in variable)
    elif clear_variables:
        variable = {}
    else:
        variable = None
    return variable


@job.command(short_help='Modify the parameters of an existing job.')
@click.argument('job')
@click.option('--name', help="Job name.")
@click.option('--command', help='The command to use for this job.')
@click.option('--schedule', help='An optional cron schedule string for this job.')
@click.option('--resource-profile', help='The resource profile to use for this job.')
@click.option('--variable', multiple=True, help='A variable setting in the form <key>=<value>. Multiple --variable options can be supplied.')
@click.option('--clear-variables', is_flag=True, help='Clear any variables in the job.')
@format_options()
@login_options()
def patch(job, name, command, schedule, resource_profile, variable, clear_variables):
    variable = _variables(variable, clear_variables)
    cluster_call('job_patch', job,
                 name=name, command=command, schedule=schedule,
                 variables=variable, resource_profile=resource_profile,
                 cli=True)


@job.command(short_help='Create a new job.')
@click.argument('project')
@click.option('--name', type=str, required=False, help="Job name. If not supplied, it is autogenerated from the project name.")
@click.option('--command', help='The command to use for this job.')
@click.option('--schedule', help='The cron schedule string for this job. The default is to run the job exactly once.')
@click.option('--resource-profile', help='The resource profile to use for this job.')
@click.option('--variable', multiple=True, help='A variable setting in the form <key>=<value>. Multiple --variable options can be supplied.')
@click.option('--run/--no-run', default=None, help='Run the job immediately, or not. For scheduled jobs, the default is --no-run; for run-once jobs, the default is --run.')
@click.option('--wait', is_flag=True, help='Wait for the completion of the job. Relevant only if --run is active.')
@click.option('--show-run', is_flag=True, help='Display the run record instead of the job record. Relevant only if --run is active.')
@click.option('--cleanup', is_flag=True, help='Run the job immediately, wait for its completion, delete the job record, and return the run record. Implies --run, --wait, and --show-run. Only valid for run-once jobs.')
@format_options()
@login_options()
def create(project, schedule, command, name, resource_profile, variable, run, wait, show_run, cleanup):
    if run is None:
        run = not schedule or cleanup
    if cleanup:
        wait = show_run = True
        if schedule:
            raise click.UsageError('--cleanup conflicts with --schedule')
        if not run:
            raise click.UsageError('--cleanup conflicts with --no-run')
    variable = _variables(variable, False)
    name_s = ' ' + name if name else ''
    cluster_call('job_create', ident=project, id_class='project',
                 schedule=schedule, name=name, command=command,
                 resource_profile=resource_profile, variables=variable,
                 run=run, wait=wait, cleanup=cleanup, show_run=show_run,
                 prefix=f'Creating job{name_s} for {{ident}}...',
                 postfix='created.', cli=True)


@job.command(short_help='Execute an existing job.')
@click.argument('job')
@format_options()
@login_options()
def run(job):
    cluster_call('job_run', job, cli=True)


@job.command(short_help='Delete a job.')
@click.argument('job')
@click.option('--yes', is_flag=True, help='Do not ask for confirmation.')
@login_options()
def delete(job, yes):
    '''Stop a job.'''
    cluster_call('job_delete', ident=job,
                 confirm=None if yes else 'Delete job {ident}',
                 prefix='Deleting {ident}...',
                 postfix='deleted.', cli=True)

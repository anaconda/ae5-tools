...
@click.group(
    short_help="info, list, open, start, stop",
...
@click.option(
    "--editor",
    help="The editor to use. If supplied, future sessions will use this editor as well. If not supplied, uses the editor currently selected for the project.",
)
@click.option(
    "--resource-profile",
...
def start(**kwargs):
    ...
    """
    response = cluster_call("session_start", **kwargs, prefix="Starting session for {ident}...", postfix="started.")
...
def restart(**kwargs):
    ...
    wildcards. But it must match exactly one session.
    """
    result = cluster_call("session_restart", **kwargs, prefix="Restarting session {ident}...", postfix="restarted.")

...
@click.group(
    short_help="activity, collaborator, delete, deploy, deployments, download, image, info, jobs, list, patch, revision, run, runs, schedule, sessions, status, upload",
...
@click.option(
    "--filename",
    default="",
    help="Filename to save to. If not supplied, the filename is constructed from the name of the project.",
)
@global_options
def download(**kwargs):
...
@click.option(
    ...
    type=str,
    required=False,
    help="Name for the job. If supplied, the name must not be identical to an existing job or run record, unless --make-unique is supplied. If not supplied, a unique name will be autogenerated from the project name.",
)
@click.option(
    "--make-unique",
    is_flag=True,

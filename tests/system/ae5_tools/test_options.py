...
def test_owner(user_session, project_list):
    ...
    first_list = None
    for cmd in (f"project list {uname}/*", f"project list --filter owner={uname}"):
        plist = _cmd(cmd)
...
def test_name(project_dup_names):
    ...
    first_list = None
    for cmd in (f"project list {pname}", f"project list */{pname}", f"project list --filter name={pname}"):
        plist = _cmd(cmd)
        if first_list is None:
            assert len(plist) > 1
...
def test_boolean_pipe(user_session, project_dup_names):
    ...
    first_list = None
    for cmd in (
        f"project list --filter name={pname}|owner={uname}",
        f"project list --filter owner={uname}|name={pname}",
    ):
        plist = _cmd(cmd)
        if first_list is None:
...
def test_boolean_comma(user_session, project_dup_names):
    ...
    for cmd in (
        f"project list --filter name={pname}|name={pname2},owner={uname}",
...
def test_columns(user_session):
    uname = user_session.username
    for cmd in (
        f"project list --columns name,editor,id --filter owner={uname}",

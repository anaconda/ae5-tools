...
def cli_deployment(cli_project):
    ...
    dname = 'testdeploy'
    ename = 'testendpoint'
    drec = _cmd('project', 'deploy', f'{prec["owner"]}/{prec["name"]}', '--name', dname, '--endpoint', ename, '--command', 'default', '--private')
...
def test_deploy_duplicate(cli_deployment):
    ...
    dname = drec['name'] + '-dup'
    with pytest.raises(CMDException) as excinfo:
        _cmd('project', 'deploy', prec["id"], '--name', dname, '--endpoint', drec["endpoint"], '--command', 'default', '--private', '--wait')
